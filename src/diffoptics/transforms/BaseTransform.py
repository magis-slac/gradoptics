...
class BaseTransform(abc.ABC):
    ...
    def apply_transform(self, rays: optics.Rays):
        ...
        new_d = torch.matmul(self.transform.type(rays.directions.dtype).to(rays.device),
                             torch.cat((rays.directions, torch.zeros((rays.directions.shape[0], 1),
                                        device=rays.device)), dim=1).unsqueeze(-1))[:, :3, 0]
        return optics.Rays(new_o, new_d, luminosities=rays.luminosities, meta=rays.meta, device=rays.device)

    def inverse_transform(self, rays: optics.Rays):
